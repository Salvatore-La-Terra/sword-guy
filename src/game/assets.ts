...
export function createGeneratedAssets(scene: Phaser.Scene) {
  if (scene.textures.exists('player-knight')) return;

  withGraphics(scene, graphics => {
    graphics.fillStyle(0x475569).fillRect(0, 0, 32, 32);
    graphics.lineStyle(1, 0x64748b, 0.5);
    for (let y = 0; y < 32; y += 8) graphics.lineBetween(0, y, 32, y);
    for (let x = 0; x < 32; x += 8) graphics.lineBetween(x, 0, x, 32);
  ...
  withGraphics(scene, graphics => {
    ...
    graphics.lineStyle(2, 0x64748b).strokeRoundedRect(2, 2, 60, 60, 7);
  ...
  withGraphics(scene, graphics => {
    graphics.lineStyle(4, 0xfbbf24, 0.95).strokeCircle(18, 18, 14);
    graphics.lineStyle(2, 0xffffff, 0.8).lineBetween(8, 18, 28, 18);
